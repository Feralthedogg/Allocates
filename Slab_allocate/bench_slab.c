...
int main(void) {
    ...
    size_t total_objects = iterations;  // Total number of objects equals iterations.
    ...
    // Allocate an array for pointers with 16-byte alignment.
    void** allocations = (void**) _aligned_malloc(iterations * sizeof(void*), 16);
    if (allocations == NULL) {
